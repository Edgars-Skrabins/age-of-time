...
namespace Dan.Main
{
    public static class LeaderboardCreator
    {
        ...
        private const string FORM_PUBLIC_KEY = "publicKey",
            FORM_USERNAME = "username",
            FORM_SCORE = "score",
            FORM_EXTRA = "extra",
            FORM_USER_GUID = "userGuid";
        ...
        private static void Initialize()
        {
            ...
            if (LeaderboardCreatorBehaviour.Config.authSaveMode != AuthSaveMode.Unhandled) _behaviour.Authorize(OnAuthorizationAttempted);
        }
        ...
        public static void Ping(Action<bool> isOnline)
        {
            _behaviour.SendGetRequest(GetServerURL(), isOnline, null);
        }
        ...
        ///     The public key of the leaderboard
        ///     (retrieve from https://danqzq.itch.io/leaderboard-creator).
        /// </param>
        ...
        public static void GetLeaderboard(
            string publicKey,
            LeaderboardSearchQuery searchQuery,
            Action<Entry[]> callback,
            Action<string> errorCallback = null)
        {
        ...
        /// <param name="errorCallback">Returns an error message if the request failed.</param>
        public static void UploadNewEntry(
            string publicKey,
            string username,
            int score,
            ...
            Action<string> errorCallback = null)
        {
            ...
            {
                if (!isSuccessful) LogError("Uploading entry data failed!");
                else Log("Successfully uploaded entry data to leaderboard!");
            };

            _behaviour.SendPostRequest(
                GetServerURL(Routes.Upload),
                Requests.Form(
                    ...
                    Requests.Field(FORM_USER_GUID, UserGuid)
                ),
                callback,
                errorCallback
            );
        ...
        public static void UpdateEntryUsername(
            string publicKey,
            string username,
            Action<bool> callback = null,
            Action<string> errorCallback = null)
        {
            ...
            callback += isSuccessful =>
            {
                if (!isSuccessful) LogError("Updating entry's username failed!");
                else Log("Successfully updated player's username!");
            };

            _behaviour.SendPostRequest(
                GetServerURL(Routes.UpdateUsername),
                Requests.Form(
                    Requests.Field(FORM_PUBLIC_KEY, publicKey),
                    Requests.Field(FORM_USERNAME, username),
                    Requests.Field(FORM_USER_GUID, UserGuid)
        ...
        public static void DeleteEntry(string publicKey, Action<bool> callback = null, Action<string> errorCallback = null)
        {
            ...
            callback += isSuccessful =>
            {
                if (!isSuccessful) LogError("Deleting entry failed!");
                else Log("Successfully deleted player's entry!");
            };

            _behaviour.SendPostRequest(
                GetServerURL(Routes.DeleteEntry),
                Requests.Form(Requests.Field(FORM_PUBLIC_KEY, publicKey), Requests.Field(FORM_USER_GUID, UserGuid)),
                callback,
                errorCallback
            );
        }
        ...
        public static void GetPersonalEntry(string publicKey, Action<Entry> callback, Action<string> errorCallback = null)
        {
            ...
            _behaviour.SendGetRequest(
                GetServerURL(Routes.GetPersonalEntry, $"?publicKey={publicKey}&userGuid={UserGuid}"),
                callback,
                errorCallback
            );
        }
